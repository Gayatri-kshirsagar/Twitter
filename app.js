...
app.get("/user/followers/",authenticationToken, async (request,response) => {
    ...
    const getUserIdQuery = `select user_id from user where username='${username}';`; 
    const getUserId = await database.get(getUserIdQuery);
    const getFollowerIdsQuery = `select follower_user_id from follower where 
...
app.get (
    ...
    async (request,response) => {
        const {tweetId} = request.params;
        let {username} = request;
        const getUserIdQuery = `select user_id from user where username username = '${username}';`;
        const getUserId = await database.get(getUserIdQuery);
    ...
    if (getTweetIds.includes(parseInt(tweetId))) {
        ...
        const getLikedUserNamesArray = await database.all(getFollowingIdsQuery);
        
        const getLikedUserNames = getFollowingIdsArray.map((eachUser) => {
...
const convertUserNameReplyedDBObjectToResposeObject = (dbObject) =>{
    return{
        replies:dbObject;
    };
};
...
app.get(
    ...
    async (request,response) => {
        ...
        if(getTweetIds.includes(parseInt(tweetId))){
            ...
            response.send(
                convertUserNameReplyedDBObjectToResposeObject(getUsernameReplyTweets
                );
            }else{
                response.status(401);
                response.send("Invalid Request");
